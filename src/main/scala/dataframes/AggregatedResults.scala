...
class AggregatedResults(spark: SparkSession, databaseUrl: String, products: Products, sales: Sales, aggregateByColumns: Map[String, Boolean]) extends BaseDataFrame {
  private val columnsToGroupBy: Array[String] = aggregateByColumns.filter(_._2).keySet.toArray
  ...
  val df: DataFrame = {
    ...
    val totalSum = calculateTotalSum(salesMatchedByRegion)
    calculateTotalSumPct(totalSum)
  }
  ...
  private def calculateTotalSum(sales: DataFrame): DataFrame = {
    sales.groupBy(columnsToGroupBy.head, columnsToGroupBy.tail: _*)
      .agg(
        round(
          sum("total_sum"),
          2
  ...
  private def calculateTotalSumPct(totalSum: DataFrame): DataFrame = {
    totalSum.withColumn(
      "total_sum_pct",
      round(
